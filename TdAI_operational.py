...
def main():
    ...
    # -------------------------------------------------------------------------
    # 📊 SECTION 5: REAL-TIME ASOS LOOP & NATIVE FILESYSTEM PORTABLE LEDGER
    # -------------------------------------------------------------------------
    ...
    if os.path.exists(output_csv_path):
        ...
        headers = ['valid_time', 'TdAI Run Time (UTC)', 'NBM Temperature (F)', 'NBM RH (%)', 'NBM Dewpoint (F)', 'TdAI Predicted Bias (F)', 'TdAI Corrected Dewpoint (F)', 'ASOS Ground Truth Dewpoint (F)', 'Raw NBM Error (F)', 'Post TdAI Error (F)', 'TdAI Skill Score (%)']
    ...
    for idx in range(len(master_input_df)):
        ...
        log_row = {
            ...
            'ASOS Ground Truth Dewpoint (F)': np.nan, 'Raw NBM Error (F)': np.nan, 'Post TdAI Error (F)': np.nan, 'TdAI Skill Score (%)': np.nan
        }

        # Check validation threshold bounds to evaluate historical accuracy
                ...
                if res.status_code == 200:
                    ...
                    if not asos_df.empty and 'dwpf' in asos_df.columns:
                        ...
                        if not valid_reports.empty:
                            ...
                            r_nbm_err = log_row['NBM Dewpoint (F)'] - asos_gt
                            p_tdai_err = log_row['TdAI Corrected Dewpoint (F)'] - asos_gt
                            skill_score = (1.0 - (abs(p_tdai_err) / abs(r_nbm_err))) * 100 if abs(r_nbm_err) > 0 else 0.0
                            
                            log_row['ASOS Ground Truth Dewpoint (F)'] = asos_gt
                            log_row['Raw NBM Error (F)'] = round(r_nbm_err, 2)
                            log_row['Post TdAI Error (F)'] = round(p_tdai_err, 2)
                            log_row['TdAI Skill Score (%)'] = round(skill_score, 1)
            except Exception as e:
                print(f"      ❌ Verification network issue: {e}")

        new_rows_list.append(log_row)

    new_entry_df = pd.DataFrame(new_rows_list)
    for col in new_entry_df.columns:
        if col not in existing_log_df.columns: existing_log_df[col] = np.nan
    ...
    # 🚀 DOUBLE-COMPOSITE DEDUPLICATION SECURITY GUARD
    combined_log_df = combined_log_df.drop_duplicates(subset=['valid_time', 'TdAI Run Time (UTC)'], keep='last')
    
    # Write directly back to local repository directory file instead of a cloud sheet API
