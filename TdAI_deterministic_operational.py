...
def process_station(station, lat, lon, target_run_hour, forecast_hours, date_strs,
                     nbm_run_hour, bulletin_text, successful_date_str,
                     grib_files, base_path):
    ...
    for fhr, (cycle_name, label) in fhr_labels.items():
        model_path = os.path.join(model_dir, f"tdai_deterministic_model_{station}_{cycle_name}.joblib")
        features_path = os.path.join(model_dir, f"deterministic_model_feature_schema_{station}_{cycle_name}.joblib")

        if not (os.path.exists(model_path) and os.path.exists(features_path)):
            print(f"⏭️ {label} ({cycle_name}): model weights or schema missing at {model_dir}. Skipping this forecast hour for K{station}.")
            continue

        print(f"   └── {label} ({cycle_name}): loading model '{os.path.basename(model_path)}' + schema '{os.path.basename(features_path)}'")
    ...
    if not models_by_fhr:
        print(f"❌ No usable models found for K{station} under {model_dir}. Skipping station entirely.")
        return
    ...
    master_input_df['TdAI Top Drivers'] = ""
    ...
    for fhr, cfg in models_by_fhr.items():
        ...
        if passing_rows.empty:
            print(f"⏭️ {cfg['label']} ({cfg['model_filename']}): no rows passed threshold gating, model not invoked.")
            continue
        ...
        # --- SHAP EXPLAINER: top 5 features driving THIS prediction ---
        # TreeExplainer is exact and fast enough to run per-row at inference
        # time (same approach as TdAI_v3.1_Deterministic_EVALUATION.py's
        # offline feature-importance section, just applied live per-forecast
        # instead of averaged across a whole holdout year).
        try:
            explainer = shap.TreeExplainer(cfg['model'])
            shap_values = explainer.shap_values(X_live)

            # sin_season/cos_season are two halves of one cyclical-encoded
            # "time of year" signal, not two independent drivers - summing
            # their signed SHAP values (rather than keeping only the larger
            ...
            for row_shap in shap_values:
                ...
                top_drivers_strs.append(driver_str)

            master_input_df.loc[fhr_mask, 'TdAI Top Drivers'] = top_drivers_strs
        except Exception as e:
            print(f"   ⚠️ SHAP explanation failed for {cfg['label']}: {e}")
        ...
        for station, (lat, lon) in STATIONS.items():
            try:
                process_station(
                    ...
                    target_run_hour=target_run_hour, forecast_hours=forecast_hours, date_strs=date_strs,
                    nbm_run_hour=nbm_run_hour, bulletin_text=bulletin_text, successful_date_str=successful_date_str,
